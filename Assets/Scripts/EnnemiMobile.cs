...
public class EnnemiMobile : MonoBehaviour
{
    ...
    [SerializeField] private float vitessePatrouille = 1.8f;
    [SerializeField] private float hauteurVague = 1.1f;
    [SerializeField] private float frequenceVague = 1.4f;

    [Header("Poursuite")]
    [SerializeField] private Transform joueur;
    [SerializeField] private float rayonDetection = 3.5f;
    [SerializeField] private float vitessePoursuite = 2.8f;

    [Header("Impact")]
    [SerializeField] private Transform pointDepartJoueur;
    [SerializeField] private float delaiEntreDegats = 1.2f;
    ...
    private void DeplacementSinusoidal()
    {
        ...
        Vector2 cibleVague = baseTrajet +
            Vector2.up * (Mathf.Sin(progressionVague * Mathf.PI * 2f) * hauteurVague);
    ...
    {
        if (!autre.CompareTag("Player") || Time.time < prochainDegat)
            return;

        prochainDegat = Time.time + delaiEntreDegats;

        // Effet de l'astéroïde
        effetAttaque?.Declencher();

        // Effet de dégâts sur le joueur
        EffetDegatsJoueur effet = autre.GetComponent<EffetDegatsJoueur>();
        if (effet != null)
        ...
        // Replace le joueur si un point est défini
